...
const findAllArt = (queryParams) => {
  ...
  return db
    .query(
      `select * FROM art as a JOIN category as c on a.category_id = c.id JOIN technique as t on a.technique_id  = t.id JOIN author on author.id = a.author_id${queryFilters}`,
      params
    )
    .then(([data]) => {
      return data;
...
const addArt = (art, img_url) => {
  ...
  return db
    .query(
      "INSERT INTO art (ref_img , full_title, short_title, achievement_date, dimensions, art_number, description, url_origin, img_url, author_id, category_id, technique_id) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [
        ref_img,
        full_title,
        short_title,
